...
export class InicioPage implements OnInit {
  ...
  cantidadPedidosTotales: number = 0;
  ...
  constructor(public auth : AuthFirebaseService,private db : ObjetoService,private cdr: ChangeDetectorRef) {
   ...
   async ngOnInit() {
    ...
    await this.obtenerCantidadTotalPedidos();
    ...
    this.porcentajeRetiros = this.porcentaje(this.cantidadPedidosTotales,this.cantidadPedidosRetirados, this.porcentajeRetiros);
    this.porcentajeEntregas = this.porcentaje(this.cantidadPedidosTotales,this.cantidadPedidosEntregado, this.porcentajeEntregas);
  }

  async obtenerCantidadTotalPedidos() {
    const uidUsuario = await this.auth.getUid();
    
    return new Promise<void>((resolve, reject) => {
      this.db.contarTotalPedidos(uidUsuario).subscribe(cantidad => {
        this.cantidadPedidosTotales = cantidad;
        console.log('Cantidad total de pedidos:', this.cantidadPedidosTotales);
        resolve();
      });
    });
  }
